#! /usr/bin/env python2
"""
A Basic Implementation of software defined network simulator in python
"""
...
def ping_switch(packet, controller):
    ...
    print "Packet Destination: ", packet.destination.id
...
def dump_switch_flow_table():
    ...
    for x in xrange(len(switch_objects)):
        ...
        if x == 0:
            print "+Left     +    NIL+"
        elif switch_objects[x].flow_table['left'] == None:
            print "+Left     +    NIL+"
        else:
            print "+Left     +     %s+" % (switch_objects[x - 1].id)
        if x == len(switch_objects) - 1:
            print "+Right    +    NIL+"
        elif switch_objects[x].flow_table['right'] == None:
            print "+Right    +    NIL+"
        else:
            print "+Right    +     %s+" % (switch_objects[x + 1].id)
        print "+++++++++++++++++++"
...
if __name__ == "__main__":
    ...
    """
    print help
    number_of_switches = int(raw_input("SDN Simulator > Enter the number of switches to create: "))
    ...
    while True:
        ...
        # If the user is trying to print the help instructions
